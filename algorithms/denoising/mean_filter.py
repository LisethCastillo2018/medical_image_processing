...
def mean_filter(image, initial_position):

    """
    Desc: Aplica el filtro de media a una imagen para la remoción de ruido.
    Params:
    ...
    while cluster and len(cluster) < np.prod(dims):
        ...
        neighborhood_mean = np.mean([image[p] for p in (neighbors + [point])])
        filtered_image[point] = neighborhood_mean
